...
c1, c2 = st.columns(2)
c1.metric("Max pain (front expiry)", fmt_strike(mp_strike))
c2.metric("Walls within band", str(len(walls)))
...
with st.expander("Notes"):
    st.markdown(
        "- Walls are ranked by **|gamma · OI|** rather than raw OI — far-OTM OI is dead weight "
        "  for dealer hedging.\n"
        "- **Max pain** alone is mostly folklore. Treat it as a *confluence* signal: when it "
        "  lines up with the zero-gamma flip and the dominant gamma wall, the level matters."
    )
